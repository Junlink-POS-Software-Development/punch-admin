...
--
-- PURPOSE:
-- 1. Secures `daily_item_stats` so tenants/stores can only view item statistics
--    for stores they are authorized to access.
-- 2. Allows store owners, co-admins, assigned staff, and platform admins to view stats.
-- 3. Protects product performance aggregation data from unauthorized client-side
--    tampering, while permitting SECURITY DEFINER functions, triggers, and admins
--    to manage records.
-- 4. Creates recommended indexes for high-speed item-level and date-range lookups.
-- ==============================================================================
...
-- (Triggers / functions with SECURITY DEFINER bypass RLS automatically)
-- ------------------------------------------------------------------------------
