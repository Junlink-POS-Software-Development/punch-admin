...
--
-- PURPOSE:
-- 1. Secures `daily_store_stats` so tenants/stores can only view their own stats.
-- 2. Allows store owners, co-admins, assigned staff, and platform admins to view stats.
-- 3. Protects aggregation data from unauthorized client-side tampering while
--    permitting background trigger functions (SECURITY DEFINER) and admins to manage records.
-- 4. Creates recommended indexes for high-speed multi-store and date-range filtering.
-- ==============================================================================
...
DROP POLICY IF EXISTS "Manage Daily Store Stats" ON public.daily_store_stats;
...
TO authenticated
USING (
  -- A. Fast path: JWT session claims match store_id
  store_id = ((auth.jwt() -> 'app_metadata' ->> 'store_id')::uuid)
  OR
  -- B. User is an admin (via JWT claim or users table)
  ...
  OR
  -- C. User is the store owner or a co-admin
  EXISTS (
  ...
  OR
  -- D. User is assigned to this store in users table
  EXISTS (
    SELECT 1 FROM public.users u
...
-- (Triggers with SECURITY DEFINER bypass RLS automatically)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 7. Performance Indexes (Ensures fast lookups and avoids table scans during RLS checks)
-- ------------------------------------------------------------------------------
